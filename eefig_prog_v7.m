...
Xk = data2(:, 1:end-1);
Yk = data2(:, end);
prc_idx = @(x) 0.1258*exp(-0.0701*x(:));
prc_idx = @(x) 0.0958*exp(-0.0468*x(:))-0.0035;
prc_idx = @(x) 0.1018*exp(-0.0471*x(:))-0.0035;

Xk = [Xk prc_idx(1:length(Yk))];
...
%         for j=i-zeta+1:i
%             P1=P{ngran};
%             [~,~,P2]=rls_step3(P1,Yk(j),Xk(j,:),theta0,g(k),ff);
%             P{ngran}=P2;
%         end
...
%             yk=flipud(data(i-buffer+1:i,1));
        ...
        if i>20
            if OFFSET
                ...
                [rul(i-buffer,:),xp]=predictRUL(EEFIG,[Xk(i,:)],0,thr,OFFSET,nu0,rho_nu,xk,pdx,i,prc_idx,Yk(i:end));
...
X = 1:97;
Y = Yk(X)+0.0035;
theta = [ones(length(X),1) X']\log(Y);
a = exp(theta(1));
b = theta(2);
hold on;
plot(X, Y);
X = 1:100;
plot(X, a*exp(b*X)-0.0035);
hold off;